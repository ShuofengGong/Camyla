...
class SemanticScholarClient(BaseLiteratureAPI):
    ...
    def __init__(self,
        ...
        self.phase1_min_year_filter = phase1_min_year.split('-')[0] + "-"  # Phase 1 year filter
    ...
    def search(self, query: str, limit: int = 10, **kwargs) -> List[Paper]:
        ...
        Args:
            ...
            **kwargs: Additional parameters:
                - phase1_mode: bool; when True, disables fields-of-study filter and
                  uses the more permissive Phase 1 year.
        ...
        # Phase 1 mode: disable fields-of-study filter and use the permissive year
        phase1_mode = kwargs.get('phase1_mode', False)

        if phase1_mode:
            use_fields_of_study = None  # Disable fields-of-study filter
            use_year_filter = self.phase1_min_year_filter  # Configured Phase 1 year
            print(f"       🔬 Phase 1 mode: fields-of-study filter disabled, year={use_year_filter}")
        else:
            use_fields_of_study = self.fields_of_study
            use_year_filter = self.min_year_filter
        ...
        try:
            ...
            params = {
                ...
                "year": use_year_filter,  # Year filter
            }
            ...
            # Add the fields-of-study filter (skipped in Phase 1 mode)
            if use_fields_of_study:
                params["fieldsOfStudy"] = use_fields_of_study
            ...
            raise  # Let the backoff decorator catch and retry
    ...
    def get_citations(self, paper_id: str, limit: int = 100) -> List[Dict]:
        ...
        Returns:
            List[Dict]: List of citing papers (containing citingPaper info).
        """
        try:
            params = {
                "fields": "title,year,authors,url,venue,publicationVenue,journal,abstract,citationCount,isOpenAccess,openAccessPdf",
                "limit": limit
            }
            ...
            citations_data = response.json().get("data", [])

            # Extract citingPaper entries
            citations = []
    ...
    def get_recommendations(self, paper_id: str, limit: int = 5) -> List[Paper]:
        ...
        try:
            ...
            papers = []
            for data in raw_papers:
                papers.append(self._create_paper_from_data(data))
    ...
    def download_pdf(self, paper_id: str, output_dir: str = ".") -> Optional[str]:
        ...
        Returns:
            Optional[str]: Path to the downloaded file, or None on failure.
        """
        output_path = None
        try:
            # 1. Resolve the PDF link
            pdf_url = None

            # Fetch it from metadata first
            try:
                response = requests.get(
                    f"{self.base_url}/paper/{paper_id}",
                    headers=self.headers,
                ...
                if response.status_code == 200:
                    ...
                    if data.get("openAccessPdf", {}).get("url"):
                        pdf_url = data["openAccessPdf"]["url"]
                    title = data.get("title", f"paper_{paper_id}")
                else:
            ...
            # Add a Referer when the link is not on Semantic Scholar
            if 'semanticscholar.org' not in pdf_url:
